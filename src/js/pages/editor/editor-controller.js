...
export default controller(function EditorController() {
  ...
  ctrl.onEnter = ({
    ...
    if (!ctrl.isNew) {
      return ctrl.getArticle(router.params.get('slug')).then(init, () => {
        router.transitionTo('/');
        return false;
      });
    }
  ...
  ctrl.saveArticle = ({
    ...
    ctrl.serverErrors = null;
    const savePromise = ctrl.isNew ? articleApi.saveArticle({
      title,
      description,
      body,
      tagList
  ...
  ctrl.updateTagList = () => {
    ...
    const newTagsList = result && result.split(',').map(tag => tag.trim()).filter(tag => tag && !existingTagsList.includes(tag));
